...
function toggleProfilePanel() {
  if (profilePanel.hidden) openProfilePanel();
...
function syncUserChip() {
  const label = profileToggle.querySelector('small');
  const age = calculateAge(state.profile.birthday);
  const skillCount = (state.profile.skills || []).length;
  label.textContent = `${state.profile.mbti} · ${state.profile.zodiac} · ${age}岁 · ${skillCount}技能`;
}
...
function loadState() {
  ...
  try {
    ...
    return {
      ...
      ...parsed,
      profile: { ...defaultState.profile, ...parsed.profile, skills: Array.isArray(parsed.profile?.skills) ? parsed.profile.skills : defaultState.profile.skills },
...
function hydrateProfile() {
  ...
  profileForm.querySelectorAll('input[name="skills"]').forEach((input) => {
