...
- 用户可发布行程帖（目的地、时间、预算、标签、景点、详细行程安排），并支持 AI 自动生成文字版具体行程安排。
- 行程支持点赞、评论（含头像、删除权限）、发布时间展示，并支持“综合/最新/最热”排序。
- 新增社交关系与聊天机制：点击行程卡头像可查看个人信息遮罩层并执行关注/取消关注、拉黑/取消拉黑、发起聊天；主页不常驻聊天模块，发起聊天后才会出现独立聊天页签（含“+”创建群组）；陌生人私聊仅 1 条消息，互相关注后可无限私聊。
- 新增消息页签：移动到首页右侧消息导航栏，支持查看聊天会话列表与系统消息（新增关注、谁赞了你的行程/主页、谁评论了你的行程）；行程支持带头像评论、评论删除（本人可删，行程作者可清空全部评论），主页支持点赞。
